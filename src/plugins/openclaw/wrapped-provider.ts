 ...
 * Not persisted — rebuilt on plugin load.
 */
 ...
 * OpenClaw's PI SDK defines this, but we keep it loose to avoid a hard dependency.
 */
...
export class WrappedProvider {
  ...
  private ensureSession(model: string): SessionState {
    ...
    if (this.session && (now - this.session.lastCallTimestamp) > this.config.session_timeout_ms) {
      // Session timed out — finalize and create new
      try {
    ...
    if (!this.session) {
      try {
        ...
        logError(e);
        // Create a fallback in-memory session
        this.session = {
  ...
  async chat(request: OpenClawProviderRequest): Promise<OpenClawProviderResponse> {
    ...
    try {
      ...
      // Track recent interactions for circuit breaker (keep last 20)
      ...
      // 9. Return response unmodified
      return response;

    } catch (error) {
      // Fail-safe: if anything in our wrapper fails, try the real provider directly
      logError(error);
      try {
        return await this.realProvider.chat(request);
      } catch (providerError) {
        throw providerError; // Only throw real provider errors
      }
    }
  }
  ...
  async *chatStream(request: OpenClawProviderRequest): AsyncIterable<any> {
    ...
    // Circuit breaker check before streaming
    try {
      const blockMsg = this.runCircuitBreaker(session);
      if (blockMsg) {
    ...
    // Stream from real provider, accumulating for logging
    let fullContent = '';
    let finalTokensIn = 0;
    ...
    try {
      ...
      logError(error);
      throw error; // Re-throw stream errors — they're from the real provider
    }
